...
<head>
    ...
    <title>Edit Organisasi - {{ $organization->name }} - {{ config('app.name', 'Laravel') }}</title>
    ...
    <div class="flex flex-1">
        ...
        <main class="flex-1 p-8 bg-gray-50">
            <div class="bg-white rounded-xl card-shadow p-8 border border-gray-100">
                ...
                <form action="{{ route('organizations.update', $organization->id) }}" method="POST" class="space-y-6">
                    @csrf
                    ...
                    <div>
                        ...
                        <input type="text" name="name" id="name" class="mt-1 block w-full px-4 py-2 border border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" placeholder="Contoh: Himpunan Mahasiswa Teknik Informatika" value="{{ old('name', $organization->name) }}" required>
                        @error('name')
                    ...
                    <div>
                        ...
                        <input type="text" name="code" id="code" class="mt-1 block w-full px-4 py-2 border border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" placeholder="Contoh: HMTI" value="{{ old('code', $organization->code) }}" required>
                        @error('code')
                            <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <textarea name="description" id="description" rows="5" class="mt-1 block w-full px-4 py-2 border border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" placeholder="Deskripsi lengkap tentang organisasi ini...">{{ old('description', $organization->description) }}</textarea>
                        @error('description')
                            <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
