    ...
    <div x-data="{ showDeleteModal: false, deleteUrl: '', deleteName: '' }">
        <div class="bg-white rounded-xl card-shadow p-6 sm:p-8 border border-gray-100"> {{-- Padding responsif --}}
            <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-6 sm:mb-8 pb-4 border-b border-gray-200"> {{-- Tata letak responsif untuk header --}}
                ...
                <a href="{{ route('users.create') }}" class="submit-button-gradient text-white font-semibold py-2 px-5 sm:py-3 sm:px-7 rounded-md shadow-md hover:shadow-lg transition duration-300 ease-in-out transform hover:-translate-y-0.5 flex items-center space-x-2 text-sm sm:text-base"> {{-- Ukuran tombol dan teks responsif --}}
            ...
            @if($users->isEmpty())
                <div class="bg-blue-50 border border-blue-200 text-blue-800 px-6 py-10 sm:px-8 sm:py-12 rounded-xl text-center card-shadow"> {{-- Padding responsif --}}
                    ...
                    <h3 class="text-xl sm:text-2xl font-semibold mb-2">Belum Ada Admin Terdaftar</h3> {{-- Ukuran H3 responsif --}}
                    <p class="text-base sm:text-lg">Silakan klik tombol "Tambah Admin Baru" di atas untuk menambahkan yang pertama.</p> {{-- Ukuran P responsif --}}
                </div>
            @else
                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                        ...
                        <tbody class="bg-white divide-y divide-gray-100">
                            ...
                            <tr class="group transition-all duration-200 ease-in-out hover:bg-gray-50">
                                ...
                                <td class="px-4 py-4 sm:px-6 sm:py-5 whitespace-nowrap text-xs sm:text-sm text-gray-700"> {{-- Teks email sedikit lebih kecil --}}
                                    {{ $user->email }}
                                </td>
                                <td class="px-4 py-4 sm:px-6 sm:py-5 whitespace-nowrap text-center text-sm font-medium">
                                    <div class="flex items-center justify-center space-x-2 sm:space-x-3"> {{-- Jarak tombol responsif --}}
                                        <a href="{{ route('users.edit', $user->id) }}" class="inline-flex items-center p-1 sm:p-2 rounded-full text-white bg-green-500 hover:bg-green-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500 transition duration-150 ease-in-out" title="Edit"> {{-- Padding tombol responsif --}}
                                        ...
                                        <button
                                            ...
                                            title="Hapus"> {{-- Padding tombol responsif --}}
                                            <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 sm:h-5 sm:w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"> {{-- Ukuran ikon responsif --}}
